...
export async function callChatGPT(api: ChatGPTAPI, content: string, retryOn503: number): Promise<any> {
    let cnt = 0;
...
export function startConversation(api: ChatGPTAPI, res: ChatMessage, retryOn503: number): any {
    return {
        res,
        retryOn503,
            ...
            while (cnt++ <= retryOn503) {
                try {
                    const response = await api.sendMessage(message,
                        { conversationId: res.conversationId, parentMessageId: res.id });
                    return response;
                } catch (err: any) {
                    if (!is503or504Error(err)) throw err;
